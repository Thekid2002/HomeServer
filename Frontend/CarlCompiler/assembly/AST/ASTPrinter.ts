...
export class ASTPrinter implements ASTVisitor<void> {
    ...
    visitAssignment(statement: Assignment): void {
        this.tree.push(
            this.getSpace(this.number) + this.number.toString() + ": Assignment"
        );
        this.number++;
    ...
    visitProgram(statement: Program): void {
        this.tree.push(
            this.getSpace(this.number) + this.number.toString() + ": Program"
        );
        this.number++;
        if (statement.body !== null) {
      statement.body!.accept<void>(this);
    ...
    visitStatementType(statement: StatementType): void {
        ...
        this.number.toString() +
        ": StatementType " +
        StatementTypeNames[statement.type]
        );
    }

    visitIfStatement(statement: IfStatement): void {
        this.tree.push(
            this.getSpace(this.number) + this.number.toString() + ": IfStatement"
        );
        this.number++;
        statement.condition.accept<void>(this);
        if (statement.body !== null) {
    ...
    visitReturn(statement: Return): void {
        this.tree.push(
            this.getSpace(this.number) + this.number.toString() + ": Return"
        );
        this.number++;
        statement.expression.accept<void>(this);
        this.number--;
