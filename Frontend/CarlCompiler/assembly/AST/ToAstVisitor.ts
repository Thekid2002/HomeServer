...
export class ToAstVisitor implements ParseVisitor<AbstractNode> {
    ...
    visitImport(statement: ParseImport): AbstractNode {
        const parentPath = statement.parentPath.accept<AbstractNode>(
            this
        ) as ASTString;
        const childPath = statement.childPath.accept<AbstractNode>(this) as ASTString;
        const functionDeclarationWithoutBody =
      statement.functionDeclarationWithoutBody.accept<AbstractNode>(
          this
    ...
    visitReturn(statement: ParseReturn): AbstractNode {
        ...
        if (statement.expression !== null) {
            expression = statement.expression.accept<AbstractNode>(
                this
    ...
    visitFunctionCallExpression(
        ...
        const functionName = expression.functionName.literal;
        const actualParameters = new Array<AbstractExpression>();
        for (let i = 0; i < expression.actualParameters.length; i++) {
            actualParameters.push(
        expression.actualParameters[i].accept<AbstractNode>(
            this
        ) as AbstractExpression
            );
        }
    ...
    visitFunctionCallStatement(
        statement: ParseFunctionCallStatement
    ): AbstractNode {
        const functionName = statement.functionName.literal;
        const actualParameters = new Array<AbstractExpression>();
        for (let i = 0; i < statement.actualParameters.length; i++) {
            actualParameters.push(
        statement.actualParameters[i].accept<AbstractNode>(
            this
        ) as AbstractExpression
            );
        }
    ...
    visitFunction(statement: ParseFunctionDeclaration): AbstractNode {
        const returnType = statement.returnType.accept<AbstractNode>(
            this
        ) as AbstractType;
        const name = statement.name.accept<AbstractNode>(this) as ASTIdentifier;
        const parameters = new Map<string, AbstractType>();
        for (let i = 0; i < statement.parameters.keys().length; i++) {
            const key = statement.parameters.keys()[i];
            parameters.set(
                (key.accept<AbstractNode>(this) as ASTIdentifier).value,
        statement.parameters
            .get(key)
            .accept<AbstractNode>(this) as AbstractType
            );
        }
        const body =
      statement.body != null
          ? (statement.body!.accept<AbstractNode>(this) as AbstractStatement)
          : null;
    ...
    visitIfStatement(statement: ParseIfStatement): AbstractNode {
        const condition = statement.condition.accept<AbstractNode>(
            this
        ) as AbstractExpression;
    ...
    visitAssignment(statement: ParseAssignment): AbstractNode {
        const identifier = statement.identifier.accept<AbstractNode>(
            this
        ) as ASTIdentifier;
        const expression = statement.expression.accept<AbstractNode>(
            this
        ) as AbstractExpression;
    ...
    visitLoopStatement(statement: ParseLoopStatement): AbstractNode {
        ...
        if (statement.initiator !== null) {
            initiator = statement.initiator!.accept<AbstractNode>(
                this
            ) as AbstractStatement;
        }
        const expression = statement.condition.accept<AbstractNode>(
            this
        ) as AbstractExpression;
        const body = statement.body.accept<AbstractNode>(this) as AbstractStatement;
    ...
    visitDeclaration(statement: ParseDeclaration): AbstractNode {
        ...
        const type = statement.type.accept<AbstractNode>(this) as ValueType;
        let expression: AbstractExpression | null = null;
        if (statement.expression !== null) {
            expression = statement.expression!.accept<AbstractNode>(
                this
    ...
    visitPowExpression(expression: ParsePowExpression): AbstractNode {
        ...
        const left: AbstractExpression =
      expression.primaryOrLeft.accept<AbstractNode>(this) as AbstractExpression;
        const right: AbstractExpression = expression.right!.accept<AbstractNode>(
            this
        ) as AbstractExpression;
    ...
    visitUnaryExpression(expression: ParseUnaryExpression): AbstractNode {
        ...
        const primaryOrRight: AbstractExpression =
      expression.primaryOrRight.accept<AbstractNode>(
          this
      ) as AbstractExpression;
    ...
    visitCompoundStatement(statement: ParseCompoundStatement): AbstractNode {
        const left = statement.left.accept<AbstractNode>(this);
        const right = statement.right.accept<AbstractNode>(this);
        return new CompoundStatement(
      left as AbstractStatement,
      right as AbstractStatement,
      statement.lineNum
        );
    }
