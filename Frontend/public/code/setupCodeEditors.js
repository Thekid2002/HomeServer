...
function startResizeHeight(e) {
    e.preventDefault();
    originalSize = parseFloat(
        getComputedStyle(terminalWindow, null)
...
function startResizeWidth(e) {
    e.preventDefault();
    originalSize = parseFloat(
        getComputedStyle(codeEditorWindow, null)
...
function resizeHeight(e) {
    const newSize =
    originalSize - ((e.pageY || e.touches[0].pageY) - originalCoordinate);
...
function resizeWidth(e) {
    const newSize =
    originalSize - ((e.pageX || e.touches[0].pageX) - originalCoordinate);
